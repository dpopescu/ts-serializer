...
export class Serializer {
    ...
    public static serialize(
        ...
        Object.keys(target.prototype._serializeMap).forEach(name => {
            ...
            if (options.list) {
                if (value) {
                    dataTarget[mapName] = value.map((v: any) =>
                        this.serializeItem(options, v)
                    );
                } else {
                    dataTarget[mapName] = [];
                }
    ...
    public static deserialize(
        ...
        Object.keys(target.prototype._serializeMap).forEach(name => {
            const options: PropertyOptions =
                target.prototype._serializeMap[name];
            const rootPath: string | null =
                options.root || classOptions.root || null;
            ...
            if (
                rootPath &&
                rootPath !== '.' &&
                valueIsDefined(jsonObject[rootPath])
            ) {
            ...
            if (options.list) {
                if (value) {
                    context[name] = value.map((v: any) =>
                        this.deserializeItem(options, v)
                    );
                } else {
                    context[name] = [];
                }
